...
if _G.NexusHubLoaded then
    ...
    if CoreGui:FindFirstChild("NexusRoundTimer") then CoreGui.NexusRoundTimer:Destroy() end
end
...
local function ApplyShowRoundTimer(state)
    if state then
        ...
        roundTimerGui.Name = "NexusRoundTimer"
...
ShootButton.Size = UDim2.new(0, 90, 0, 40)
ShootButton.Font = Enum.Font.GothamBold
ShootButton.Text = "Sweet's AIM"
ShootButton.TextColor3 = Theme.Accent
...
RunService.Heartbeat:Connect(function(dt)
    ...
    if movementAccumulator < 0.10 then return end
...
local knifeHomingAccumulator = 0
RunService.Heartbeat:Connect(function(dt)
    knifeHomingAccumulator += dt
    if knifeHomingAccumulator < 0.05 then return end
    knifeHomingAccumulator = 0
...
local antiFlingAccumulator = 0
RunService.Heartbeat:Connect(function(dt)
    antiFlingAccumulator += dt
    if antiFlingAccumulator < 0.15 then return end
    antiFlingAccumulator = 0
...
local autoGrabAccumulator = 0
RunService.Heartbeat:Connect(function(dt)
    autoGrabAccumulator += dt
    if autoGrabAccumulator < 0.10 then return end
    autoGrabAccumulator = 0
...
local targetCacheAccumulator = 0
RunService.Heartbeat:Connect(function(dt)
    targetCacheAccumulator += dt
    if targetCacheAccumulator < 0.10 then return end
    targetCacheAccumulator = 0
...
RunService.Heartbeat:Connect(function(dt)
    espAccumulator += dt
    if espAccumulator < 0.05 then return end
    espAccumulator = 0
    UpdateESP()
end)
...
ToggleButton.Image = "rbxthumb://type=Asset&id=99317971916804&w=420&h=420"
...
CreateSection(TabUpdates, "📅 New Updates")
...
VersionTitle.Text = "🔄 Version 1.0 Release"
...
UpdateDate.Text = "📅 Data: August 8 2026"
...
ChangesText.Text = "✅ NEW FEATURES:\n• 🖼️ Button with custom image\n• 🔪 Knife Silent Aim\n• 🎯 Gun Silent Aim\n• 🌀 Updated FLING (with timeout)\n• ❤️ Second Life (Godmode)\n• 🔝 Notifications always on top\n• 🎯 Tracer for Gun Drop\n• 👻 Tracer for Ghost Part (native)\n• 🔄 Sweet's AIM unequips weapon\n• 💣 Auto Prank Bomb\n• 🛡️ Anti Silent Aim (Alternative Desync)\n\n🔄 IMPROVEMENTS:\n• ⚡ General optimization\n• 🔧 Bug fixes\n• 🎨 Improved interface\n\n⚠️ NOTICE:\n• Gun Silent Aim: 100% focus on Murderer\n• Knife Silent Aim: Focuses on everyone (ignores walls, aims at the center of the screen)\n• Anti Silent Aim can be activated with a key (default X) or through the menu."
...
task.wait(1)
NexusNotify("SweeTBloxY Hub MM2 V1.0",
